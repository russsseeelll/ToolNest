<div class="w-full">
    <div class="flex flex-col lg:flex-row lg:h-[40rem] space-y-4 lg:space-y-0 lg:space-x-4">
        ...
        <div class="w-full lg:w-1/4">
            <div class="bg-white border border-gray-300 rounded-lg shadow-lg p-6 h-full flex flex-col">
                ...
                <form method="GET" action="{{ route('manage') }}" class="mb-4">
                    <!-- Force the tool-manager tab -->
                    <input type="hidden" name="tab" value="tool-manager">
                    <div class="flex">
                        <input
                            type="text"
                            name="search"
                        ...
                        />
                        <button type="submit" class="bg-[#003865] hover:bg-[#002a52] text-white px-4 py-2 rounded-r-lg">
                            Search
                        </button>
                    </div>
                </form>
                    ...
                    @foreach($tools as $toolItem)
                        <li class="flex justify-between items-center bg-gray-50 p-3 rounded-lg border border-gray-300 shadow-sm">
                            ...
                            <div class="flex space-x-2">
                                <a href="{{ route('manage.tool', array_merge(request()->all(), ['tool' => $toolItem->id, 'tab' => 'tool-manager'])) }}"
                                   class="bg-[#003865] hover:bg-[#002a52] text-white px-4 py-2 rounded-lg transition">
                                    Edit
                                </a>
                                <form action="{{ route('tools.destroy', $toolItem->id) }}" method="POST" class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="bg-[#7d2239] hover:bg-[#5c1a2b] text-white px-4 py-2 rounded-lg transition">
                                        Delete
                                    </button>
                                </form>
                            </div>
                ...
                <!-- Pagination Links (only if $tools is a paginator) -->
                @if(method_exists($tools, 'links'))
                    <div class="mt-4">
                        {{ $tools->appends(array_merge(request()->all(), ['tab' => 'tool-manager']))->links() }}
                    </div>
                @endif

                <!-- Add New Tool Button -->
                <div class="mt-4 sticky bottom-0 bg-white pt-2">
                    <a href="{{ route('manage', array_merge(request()->all(), ['tab' => 'tool-manager'])) }}"
                       class="block bg-[#385a4f] hover:bg-[#2c483d] text-white p-3 rounded-lg shadow-md text-center transition">
                        Add New Tool
                    </a>
                </div>
            </div>
        ...
        <div class="w-full lg:w-3/4">
            <div class="bg-white border border-gray-300 rounded-lg shadow-lg p-6 h-full overflow-y-auto">
                ...
                <form id="tool-form" action="{{ isset($tool) ? route('tools.update', $tool->id) : route('tools.store') }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="flex justify-end space-x-2">
                        ...
                        @endif
                        <button type="submit" class="bg-[#003865] text-white px-4 py-2 rounded-lg">
